...
const countSort = (inputArr, n = inputArr.length) => {
    ...
    temp = new Array(k + 1).fill(0);
	 count = new Array(k + 1).fill(0);
	...
	document.getElementById('time').textContent=tottime+" S"
	console.log(tottime)
	return outputArr;
  }
...
submit.addEventListener('click',async function(){
    let start=document.getElementById('exampleFormControlFile1').value;
    let stop=document.getElementById('exampleFormControlFile2').value;
    let c= count[stop]-count[start-1];
    document.getElementById('counting').textContent="number of digits in a given range is:"+c;
})